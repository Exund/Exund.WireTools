...
namespace Exund.WireTools
{
    internal class Logger
    {
        ...
        {
            UnityEngine.Debug.Log($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.ffff")} | {(LogLevel)level} | {loggerID} | {message}");
        }

        private void LogException(byte level, Exception exception)
        {
            UnityEngine.Debug.LogError($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.ffff")} | {(LogLevel)level} | {loggerID} | {exception.ToString()}");
        }

        private void LogException(byte level, Exception exception, string message)
        {
            UnityEngine.Debug.LogError($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.ffff")} | {(LogLevel)level} | {loggerID} | {message}:\n {exception.ToString()}");
        }
